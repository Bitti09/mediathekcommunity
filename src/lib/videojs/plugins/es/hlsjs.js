/**
 * Modified (c) 2023 The Nuevodevel Team. All rights reserved.
 * Hls.js tech handler for video.js
 * Version 1.5.0
 */
    ...
    function Html5Hlsjs(source, tech) {
        ...
        function _handleMediaError(error) {
            ...
            if (_errorCounts[Hlsjs.ErrorTypes.MEDIA_ERROR] === 1) {
                ...
                _hls.swapAudioCodec();
                _hls.recoverMediaError();
            } else if (_errorCounts[Hlsjs.ErrorTypes.MEDIA_ERROR] > 1) {
            ...
            if (data.fatal) {
                switch (data.type) {
                    ...
                    default:
                        // cannot recover
                        _hls.destroy();
                        console.info('bubble error up to VIDEOJS');
                        tech.error = function () { return error; };
                        tech.trigger('error');
                        break;
                }
        ...
        function _toggleLevel(level, toggle) {
			...
			//_hlsjsConfig.smoothQualityChange=true;
            // NOTE: Brightcove switcher works TextTracks-style (enable tracks that it wants to ABR on)
            if (qualityLevels.length>0) {
                if (typeof toggle === 'boolean') {
                    _player.qualities[level].enabled = toggle;
                }
                return _player.qualities[level].enabled;
            }
            return false;
        }
        ...
        function _handleQualityLevels() {
            ...
            if (_metadata) {
				...
				_player.qualities=[];
				qualityLevels = _player.qualities;
                ...
                for (var i = 0; i < _metadata.levels.length; i++) {
					...
					qualityLevels.push(representation);
        ...
        function _syncQualityLevels(event, data) {
           ...
           if (_player.qualities.length>0) {
			   _player.qualityIndex = data.level;
			   _player.trigger('qualityChange');
            }
        }
        ...
        function _notifyVideoQualities() {
            if (_metadata) {
				...
				_player.qualities = qualityLevels;
        ...
        function _initHlsjs() {
			...
			if(_hlsjsConfig.capLevelToPlayerSize === undefined) _hlsjsConfig.capLevelToPlayerSize=true;
			if(_hlsjsConfig.smoothQualityChange === undefined) _hlsjsConfig.smoothQualityChange=true;
			...
			if(_hlsjsConfig.capLevelToPlayerSize===true) {
			
					if(isFirefox || isChrome || isEdgeChromium) {
						if(!_hlsjsConfig.maxBufferLength) _hlsjsConfig.maxBufferLength = 60;
						if(!_hlsjsConfig.maxMaxBufferLength) _hlsjsConfig.maxMaxBufferLength = 60;
					}

			}

			if(_hlsjsConfig.liveSyncDurationCount===undefined) _hlsjsConfig.liveSyncDurationCount =3;
        ...
        this.seekable = function () {
            ...
            if (_hls.media) {
                ...
                var endTime = Math.round(_hls.media.duration - 15);
...
function nuevoHlsjsConfigHandler(options) {
    var player = this;
...
var registerConfigPlugin = function (videojs) {
  ...
  registerVjsPlugin("nuevoHls", nuevoHlsjsConfigHandler);
};
if (typeof window !== "undefined") {
  if (window.videojs) {
    registerConfigPlugin(window.videojs);
